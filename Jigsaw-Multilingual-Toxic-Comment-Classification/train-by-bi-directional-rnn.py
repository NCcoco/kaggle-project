...
from keras.api._v2.keras.layers import Dense, RNN, GRU, LSTM, SimpleRNN, \
    BatchNormalization, Embedding, Bidirectional
...
# embeddings_index = {}
# f = open('datasets/glove.840B.300d.txt','r',encoding='utf-8')
# for line in tqdm(f):
#     values = line.split(' ')
#     word = values[0]
...
#
#
# embedding_matrix = np.random.random(size=(len(word_index)+1, 300))
# for word, i in tqdm(word_index.items()):
#     embedding_vector = embeddings_index.get(word)
#     if embedding_vector is not None:
#         embedding_matrix[i] = embedding_vector
# np.save("my_embedding_matrix.npy", embedding_matrix)
...
print(f"Auc:{roc_auc(scores, y_valid):.2}")
